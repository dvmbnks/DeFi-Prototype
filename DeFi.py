...
def addInfo():
    ...
    btn_exit = CTkButton(root, text="Back to Main Menu", command=nextPhase)
    btn_exit.pack()
...
def calculate_compound_interest(principal, rate, time, result_label):
    try:
        ...
        save_calculation_to_database("Compound Interest", compound_interest)
    except ValueError:
...
def save_calculation_to_database(calculation_type, result):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO calculations (calculation_type, result) VALUES (?, ?)
...
def calculate_mortgage(loan_amount, interest_rate, loan_term, result_label):
    try:
        ...
        # Save the calculation result to the database
        save_calculation_to_database("Mortgage Payment", monthly_payment)
    except ValueError:
